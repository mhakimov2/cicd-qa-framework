...
public class WebElementHelper {
    ...
    public static void retryingClick(By by) {
        int attempts = 0;
        while (attempts < 2) {
            try {
                click(by);
...
//        ((JavascriptExecutor)DriverBase.getDriver()).executeScript("console.log(arguments[0]); var select = arguments[0]; for(var i = 0; i < select.options.length; i++){ if(select.options[i].text == arguments[1]){ select.options[i].selected = true; } }", select, "TCPBridgeProfile");
    }
